...
/*async */
fn instantiate(path: &str) -> Result<(Store<WasiCtx>, Command)> {
    println!("{}", path);
...
/*async */
fn run_hello_stdout(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
...
/*async */
fn run_panic(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
...
/*async */
fn run_args(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    store
        .data_mut()
...
/*async */
fn run_random(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    struct FakeRng;
...
/*async */
fn run_time(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
...
/*async */
fn run_stdin(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    store
        .data_mut()
...
/*async */
fn run_poll_stdin(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    store
        .data_mut()
        .set_stdin(Box::new(ReadPipe::new(Cursor::new(
            "So rested he by the Tumtum tree",
...
/*async */
fn run_env(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
...
/*async */
fn run_file_read(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
...
/*async */
fn run_file_append(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let dir = tempfile::tempdir()?;
...
/*async */
fn run_file_dir_sync(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let dir = tempfile::tempdir()?;
...
/*async */
fn run_exit_success(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let r = wasi.call_main(&mut store)/*.await*/;
...
/*async */
fn run_exit_default(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let r = wasi.call_main(&mut store)/*.await*/?;
    assert!(r.is_ok());
    Ok(())
}
...
/*async */
fn run_exit_failure(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let r = wasi.call_main(&mut store)/*.await*/;
    let err = r.unwrap_err();
    let status = err.downcast_ref::<wasi_common::I32Exit>().unwrap();
...
/*async */
fn run_exit_panic(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let r = wasi.call_main(&mut store)/*.await*/;
    let err = r.unwrap_err();
...
/*async */
fn run_directory_list(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let dir = tempfile::tempdir()?;
...
/*async */
fn run_default_clocks(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    wasi.call_main(&mut store)?
        /*.await*/
        .map_err(|()| anyhow::anyhow!("command returned with failing exit status"))
}
...
/*async */
fn run_big_random_buf(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_clock_time_get(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_close_preopen(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_overwrite_preopen(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_fd_filestat_get(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_fd_filestat_set(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_fd_readdir(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_file_pread_pwrite(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_file_truncation(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_file_unbuffered_write(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_interesting_paths(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    if cfg!(windows) {
        expect_fail(run_with_temp_dir(store, wasi) /*.await*/)
    } else {
        run_with_temp_dir(store, wasi) /*.await*/
...
/*async */
fn run_isatty(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_nofollow_errors(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_path_exists(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_path_open_create_existing(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_path_open_missing(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_path_rename_dir_trailing_slashes(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_poll_oneoff_stdio(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    if cfg!(windows) {
        expect_fail(run_with_temp_dir(store, wasi) /*.await*/)
    } else {
        run_with_temp_dir(store, wasi) /*.await*/
...
/*async */
fn run_remove_nonempty_directory(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_renumber(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_sched_yield(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_stdio(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_symlink_create(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_symlink_filestat(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}
...
/*async */
fn run_unlink_file_trailing_slashes(store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    run_with_temp_dir(store, wasi) /*.await*/
}

/*async */
fn run_export_cabi_realloc(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    wasi.call_main(&mut store)?
        /*.await*/
        .map_err(|()| anyhow::anyhow!("command returned with failing exit status"))
}
...
/*async */
fn run_read_only(mut store: Store<WasiCtx>, wasi: Command) -> Result<()> {
    let dir = tempfile::tempdir()?;
